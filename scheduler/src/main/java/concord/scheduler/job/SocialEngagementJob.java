...
public class SocialEngagementJob {
	...
	private String JMS_SOCIAL_ENGAGEMENT_ROUTE2 = "socialEngage2";

	private String AlexUsrId = "11735713";
	private String CarmenUsrId = "1";
	...
	public void executeInternal() throws Exception {
		...
		// Carmen user
		handleSocialEngagement(grouppedPhotos, JMS_SOCIAL_ENGAGEMENT_ROUTE2, CarmenUsrId);
		...
		for (List<ClassifiedPhoto> groupPhotoItems : grouppedPhotos.values()) {
			...
			for (ClassifiedPhoto classifiedPhoto : groupPhotoItems) {
				if (classifiedPhoto.getEngagementStatus() != null && !SocialEngagementStatus.WAITING
					...
					if (classifiedPhoto.getAiClassification().get(0).getHighestScore().getLevel() > PhotoCategory.STAGE2
									.getLevel()) {
						...
						if (count == limit) {
							LOGGER.info("Breaking up social engagement for this photo category. Limit has been reached [" + limit + "]");
							break;
						}

					} else {
						LOGGER.info("Skiping photo " + classifiedPhoto.getPhoto().getPhotoId()
										+ " -> classified lower than minimum level");
